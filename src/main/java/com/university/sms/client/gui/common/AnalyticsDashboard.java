...
public class AnalyticsDashboard extends JPanel {
    ...
    public void refreshData() {
        ...
        SwingWorker<Map<String, Object>, Void> worker = new SwingWorker<>() {
            ...
            protected Map<String, Object> doInBackground() {
                try {
                    int facultyId = getCurrentUserFacultyId();
                    if (facultyId <= 0) {
                        // If can't determine faculty, default to 1 or skip
                        facultyId = 1;
                    }
                    ...
                    data.put(Constants.KEY_FACULTY_ID, facultyId);
    ...
    /**
     * Lấy facultyId từ currentUser
     * - STUDENT: Lấy từ student.facultyId
     * - TEACHER: Lấy từ courses/subjects (tạm thời default 1)
     * - ADMIN: Default 1 (có thể cải thiện để cho phép chọn)
     */
    private int getCurrentUserFacultyId() {
        ...
        try {
            if (currentUser.getRole() == User.UserRole.STUDENT) {
                ...
                if (response != null && response.isSuccess()) {
                    com.university.sms.model.Student student = 
                        response.getData(Constants.KEY_STUDENT, com.university.sms.model.Student.class);
                    if (student != null) {
                        return student.getFacultyId();
                    }
                }
            } else if (currentUser.getRole() == User.UserRole.TEACHER) {
                // TODO: Get facultyId from teacher's courses/subjects
                // For now, default to 1
                return 1;
            } else if (currentUser.getRole() == User.UserRole.ADMIN) {
                // Admin can view all, default to 1
